...
from simulator.models.emnist_models import lenet5_with_dropout
...
swap_step = 1000
mode = None 
test_step = 878 # <-- number of steps between computations of test error
timer = s_utils.Timer()
...
label = '$p\in {0} {1:.2f}, ..., {2:.2f} {3}^{4} $'.format(
    '\{', 1 - min(swapnoise_list), 1 - max(swapnoise_list), '\}', n_replicas)
...
ax.plot(xnoswap_test, ynoswap_test, label='$p^*={0:.2f}$'.format(1-noise_list[noswap_minrid]),
...
top_line_y = [7, 7]
ax.plot([0, 400*EPOCH_MULT], top_line_y, linestyle='--', color='black', linewidth=2.5, dashes=(8, 12))
top_line_y = [6, 6]
ax.plot([0, 400*EPOCH_MULT], top_line_y, linestyle='--', color='black', linewidth=2.5, dashes=(8, 12))
top_line_y = [5, 5]
ax.plot([0, 400*EPOCH_MULT], top_line_y, linestyle='--', color='black', linewidth=2.5, dashes=(8, 12))
...
path = os.path.join(dirname, 'emnist-dropout.eps')
