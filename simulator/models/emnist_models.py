...
def lenet5_lr_with_const_dropout(graph, keep_prob=0.95, verbose=False):
...
def lenet5_l2_with_const_dropout(graph, keep_prob=0.99, verbose=False):
  ...
  with graph.as_default():
    with tf.device('gpu:0'):
      ...
      with tf.name_scope('inputs'):
        x = tf.placeholder(DTYPE, shape=(None, 28, 28, 1), name='x')
        y = tf.placeholder(tf.int32, shape=(None), name='y')
        #l2 = tf.placeholder(tf.float32, shape=(), name='lr')
        padded = tf.pad(x, [[0, 0], [2, 2], [2, 2], [0, 0]])
...
def lenet5_with_lr_and_const_dropout_(graph, verbose=False, keep_prob=0.95):
